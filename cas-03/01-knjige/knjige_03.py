...
Polja unutar XML tagova mogu da budu u proizvoljnom redosledu. Koristiti preduvide (lookahead).
"""
...
knjige_regex = r"<knjiga\s+rbr=\"(?P<rbr>\d+)\">\s*"\
    ...
    + r"(?=.*?<cena\s+valuta=\"(?P<valuta>[A-Za-z]{3})\">\s*(?P<cena>\d+)\s*</cena>\s*)"\
    + r".*?"\
    + r"</knjiga>"
...
for match in re.finditer(knjige_regex, sadrzaj, re.DOTALL):
    ...
    god_izdanja = match.group("god_izdanja")
